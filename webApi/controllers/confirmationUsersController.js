...
exports.confirmMail = (req, res, next) => {
    ...
    confirmationUsers.findOne({ _id: token }, (err, user) => {
        if (err) {
            ...
            if (hours <= 24) {
                signupUser(user.email, user.password, user.firstName, user.lastName, user.tel, user, profession, user.checkedIn, user.imageUrl);
            } else {
                res.status(401).send({
...
function signupUser(email, password, firstName, lastName, tel, profession, checkedIn, imageUrl) {
